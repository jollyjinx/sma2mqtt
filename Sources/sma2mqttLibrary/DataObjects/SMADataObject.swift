...
{
    private enum CodingKeys: String, CodingKey { case object, lri, Prio, TagId, TagIdEventMsg, Unit, DataFrmt, Scale, Typ, WriteLevel, GridGuard, TagHier, Min, Max, Sum, Avg, Cnt, MinD, MaxD, SumD }
    ...
    public init(from decoder: Decoder) throws
    {
        ...
        guard let lriString = try? values.decode(String.self, forKey: .lri)
        else
        {
            throw DecodingError.dataCorruptedError(forKey: .object, in: values, debugDescription: "could not decode lri:\(objectString) ")
        }
...
extension SMADataObject
{
    ...
    static func dataObjects(from jsonString: String) throws -> [String: SMADataObject]
    {
        do
        {
            ...
            let replaced = jsonString.replacing(regex)
            { match in
                """
                \(match.1)
